...
def train_projection(args, model, optimizer, criterion_Intra, epoch, meta_trainloader):
    ...
    for idx, (samples, global_targets) in enumerate(meta_trainloader):
        ...
        # =================== forward =====================
        logits_vis, logits_sem, loss_l2, output = model(samples, selected_classes, args.extract_dataset, split=meta_trainloader.dataset.split)
    ...
    for epoch in range(args.start_epoch + 1, args.epochs + 1):
        ...
        if (epoch % args.save_freq == 0) and main_process():
            ...
            if osp.exists(filename):
                os.remove(filename)
            try:
                torch.save({'epoch': epoch, 'state_dict': model.module.proj.state_dict(),
                    'optimizer': optimizer.state_dict()}, filename)
            except:
                torch.save({'epoch': epoch, 'state_dict': model.proj.state_dict(),
                    'optimizer': optimizer.state_dict()}, filename)
